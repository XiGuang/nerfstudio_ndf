...
if __name__ == '__main__':
    ...
    if args.model == 'gim_dkm':
        ...
        # t=time.time()
        # print(t)
        # dense_points0=dense_matches.reshape(-1,4)[:,:2]
        # dense_certainty0=dense_certainty.reshape(-1)
        # dense_points0=torch.stack((width0*(dense_points0[:,0]+1)/2,height0*(dense_points0[:,1]+1)/2),dim=-1)
        # dense_points0=torch.round(dense_points0).int()
        # dense_points0=torch.clip(dense_points0[:,0]+dense_points0[:,1]*width0,0,height0*width0-1)
        # dense_points0=dense_points0[dense_certainty0>0.1].long()
        # img=torch.zeros(height0*width0,dtype=torch.uint8,device=device)
        # img[dense_points0]=255
        # print(time.time()-t)
        # cv2.imshow('map',img.cpu().reshape(height0,width0).numpy())
        # cv2.waitKey(0)
        # exit(0)
    ...
    wrapped_images = warp_images(image0, image1, geom_info,
                                 "Homography")
