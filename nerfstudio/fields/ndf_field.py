...
from nerfstudio.field_components.diff2_sigma_module import Diff2SigmaModule


class NDFField(Field):
    """Compound Field
    ...
    def __init__(
        ...
        self.diff2_sigma = Diff2SigmaModule()
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[Tensor, Tensor]:
        ...
        base_mlp_out = self.mlp_base(positions_flat) # 修改
        base_mlp_out = self.mlp_2(torch.cat([base_mlp_out,positions_flat],dim=-1))
        density_before_activation = self.to_diff_density(base_mlp_out).view(*ray_samples.frustums.shape, -1)
        ...
        diff_density = self.average_init_density * trunc_exp(density_before_activation.to(positions))
        diff_density = diff_density * selector[..., None]

        self.diff_density=diff_density
        density=self.diff2_sigma(diff_density)
    ...
    def get_outputs(
        ...
        density_embedding=self.mlp_middle(density_embedding)
        ...
        outputs.update({FieldHeadNames.DIFF_DENSITY: self.diff_density})
        return outputs
